(** A concrete-syntax parser for the source language S. *)

(** Raised on malformed input. *)
exception Parse_error = S_syntax.Parse_error

(* Run entry [start] over [src], mapping ocamlyacc failures to {!Parse_error}. *)
let run_parser
...
(** Parse a single S command into a one-command program with no functions. *)
